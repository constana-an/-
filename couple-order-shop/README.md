...
`npm test` 依次执行运行时完整性检查、迁移可重放检查、日期逻辑单测（分别在美东与北京时区各跑一遍）、RLS 策略与授权检查、Sites worker 测试和 Playwright 交互测试。其中 `tests/accessibility.spec.ts` 会在五个页面上实测**对比度（正文 4.5:1、大字 3:1）**和**触控目标 44×44**，改动配色或按钮尺寸后如果它失败，是配色的问题不是测试的问题。也可以单独运行：
