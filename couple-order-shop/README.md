...
界面支持**中文 / English 切换**：身份选择页右上角和「我们」页的「语言 / Language」一行都能切，选择存在本机、按设备生效——两个人可以各读各的语言。默认中文，不会根据浏览器语言自动切换。
...
- 自己写的心愿每对情侣最多 30 个。
- 回忆页的时间线**不截断**：按月分组，超出一页用「还有 N 件，继续往前看」加载，早期的心愿不会被悄悄丢掉。
- 里程碑卡可以展开看全部已达成的记录，不只是最近一个。
...
- `src/lib/` — 纯逻辑：日期与周期键、菜单与任务目录、本地存储、错误文案、中英文案词典（`i18n.ts`）、Supabase 懒加载。
- `src/i18n.tsx` — 语言的 React context。`src/lib/` 不含框架代码，而各页面又不能反过来从 `Prototype.tsx` 导入，所以 provider 单独放在这里。
...
`npm test` 依次执行运行时完整性检查、迁移可重放检查、日期与文案单测（分别在美东与北京时区各跑一遍）、RLS 策略与授权检查、Sites worker 测试和 Playwright 交互测试。`tests/i18n.test.mjs` 保证两种语言的键完全对齐、英文不会丢掉中文里出现的 `{变量}`、每个上架条目都带英文；`tests/language.spec.ts` 覆盖切换本身，以及**双方自己写的内容不会被翻译**这条边界。其中 `tests/accessibility.spec.ts` 会在五个页面上实测**对比度（正文 4.5:1、大字 3:1）**和**触控目标 44×44**，改动配色或按钮尺寸后如果它失败，是配色的问题不是测试的问题。也可以单独运行：
